...
func (m *mockClient) ID() int                  { return m.id }
func (m *mockClient) Start(ctx context.Context) {}
func (m *mockClient) Execute(j job.Job)         {}
func (m *mockClient) Submit(j job.Job) bool {
...
func TestSchedulerDispatchWithWorkerClient(t *testing.T) {
	...
	s := scheduler.NewScheduler(pq, dq, available, wal, jobStore, dead, 10)
	...
	dispatched := s.Dispatch(ctx, j)

	if !dispatched {
		t.Fatalf("expected dispatch to succeed")
	}

	if len(mc.submitted) != 1 || mc.submitted[0].ID != 1 {
		t.Fatalf("expected job 1 submitted to worker client, got %v", mc.submitted)
	}
}
